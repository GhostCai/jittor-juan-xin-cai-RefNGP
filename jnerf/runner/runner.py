...
class Runner():
    ...
    def train(self):
        ...
        for i in tqdm(range(self.start, self.tot_train_steps)):
            ...
            gradient = self.model.gradient(pos)

            # print(gradient.shape)
            loss_g = 0
            if self.has_G and rays_d.numel == gradient.numel:
                loss_g = jt.maximum((gradient * dir).sum(-1).reshape(-1,1),0).mean()
            rgb = self.sampler.rays2rgb(network_outputs, training_background_color)

            loss = self.loss_func(rgb, rgb_target) + 2*loss_g
        ...
        print("try to save")
        print(os.path.join(self.save_path, "params.pkl"))
